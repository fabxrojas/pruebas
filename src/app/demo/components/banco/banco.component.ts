...
export class BancoComponent implements OnInit {
    bancoForm: FormGroup; // Formulario reactivo para crear/editar un banco
    bancoList: Banco[] // Lista de bancos (se llena desde el servicio)
    isEditing: boolean = false; // Indica si se está creando un nuevo banco
    editingRowIndex: number | null = null; // Índice de la fila que se edita
    editingBanco: Banco | null = null; // Banco actualmente en edición
    editingRows: { [s: string]: boolean } = {}; // Controla edición fila por fila
    editingData: any = {}; // Copia de datos editados
    displayDialog: boolean = false; // (Por si luego usas un modal)
    isNew: boolean = false; // Indica si es un registro nuevo
    clonedBancos: { [s: string]: Banco } = {} // Copias de filas para revertir cambios
    items: any[] = []; // Items para el breadcrumb
    isEditingAnyRow: boolean = false; // Bloquea edición múltiple
    rowsPerPage: number = 10; // Numero de filas por página

    // Creamos el constructor para inyectar los servicios
    constructor(
